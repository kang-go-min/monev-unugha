                        ...
                        <div class="card-block">
                            ...
                            <table class="table table-hover table-listing">
                                ...
                                <tr>
                                    ...
                                    <th is='sortable'
                                        :column="'survey_id'">{{ trans('admin.answer.columns.survey_id') }}</th>
                                    <th is='sortable'
                                        :column="'user_id'">{{ trans('admin.answer.columns.user_id') }}</th>
                                    <th is='sortable'
                                        :column="'ip_address'">{{ trans('admin.answer.columns.ip_address') }}</th>
                                    <th></th>
                                </tr>
                                <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                    <td class="bg-bulk-info d-table-cell text-center" colspan="7">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('brackets/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a
                                                    ...
                                                    @click="onBulkItemsClickedAll('/admin/answers')"
                                        ...
                                        <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                        @click="bulkDelete('/admin/answers/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tr v-for="(item, index) in collection" :key="item.id"
                                    ...
                                    <td>@{{ item.id }}</td>
                                    <td>
                                        <detail-tooltip :data="item.survey" :text="item.survey.title"
                                                        v-if="item.survey"></detail-tooltip>
                                    </td>
                                    <td><detail-tooltip :data="item.user" :text="item.user.name"
                                                        v-if="item.user"></detail-tooltip></td>
                                    <td>@{{ item.ip_address }}</td>

                                    <td>
                                        <div class="row no-gutters">
                                            <div class="col-auto">
                                                <a class="btn btn-sm btn-spinner btn-info" :href="item.resource_url"
                                                   title="{{ trans('admin.answer.btn.show') }}" role="button"><i
                                                        class="fa fa-eye"></i></a>
                                            </div>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/answers/create') }}"
                                   role="button"><i
                                        class="fa fa-plus"></i>&nbsp; {{ trans('admin.answer.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
